...
import Model.Date as Date exposing (Date, Month, compare, onlyYear)
...
--Calculeaza lungimea intervalului returnand Maybe Int, nu Maybe(Int, Int)
--Am folosit-o pentru compararea intervalelor
intervalLength : Interval -> Maybe Int
intervalLength (Interval interval) = 
...
compare (Interval intA) (Interval intB) =
    chainCompare (Basics.compare (intervalLength (Interval intA) |> Maybe.withDefault 99) (intervalLength (Interval intB) |> Maybe.withDefault 99))
                 (Date.compare intA.start intB.start)


--Afiseaza intervalul
--functia intervalLen creaza un string dintr-un interval, pentru a putea fi afisata. Daca intervalul este Nothing, atunci string-ul este gol.
view : Interval -> Html msg
view (Interval interval) =
    let
        intervalLen (Interval interval1) =
            case (length (Interval interval1)) of
        ...
        div [class "interval"] [
            ...
            ,div [class "interval-end"] [
                if (interval.end == Nothing) then
                    ...
                    Date.view (Maybe.withDefault (Date.onlyYear 2001) interval.end)
            ]
            , div [class "interval-length"] [intervalLen (Interval interval)]
        ]
